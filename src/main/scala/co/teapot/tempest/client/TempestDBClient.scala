...
import co.teapot.tempest.{Node => ThriftNode, TempestDBService}
...
// Example scala TempestClient
object TempestDBClient {
  def main(args: Array[String]) = {
    val client = new TempestDBClient("localhost", 10001)
    println ("Node  1 has indegree " +
      client.getExecutor.inDegree("has_read", new ThriftNode("users", "alice")) +
      " and outdegree " + client.getExecutor.outDegree("has_read", new ThriftNode("users", "alice")))
    client.close()
  }
}
